	...
	while(halt == 0) {
		...
		switch(parsecommand(token)) {
			...
			case SIZE: {
				...
				if(arg1 == NULL) {
					printf(" No entry name specified.\n");
				} else {	
					res = size(currentcluster, arg1);
				}
...
int cd(uint &currentcluster, uint &parentcluster, char* dir_name) {
	...
	DIR = setdir(currentcluster,dir_name);
...
int mkdir(uint currentcluster, char* dir_name) {
	long offset;
	uint newcluster;
	char dname[12];
	int temp;
	...
	DIR = getdir(currentcluster, dname);
	if(DIR.DIR_Name[0] == 0) {
		offset = getunusedentry(currentcluster);

		for(int i = 0; i < 11; i++) {
			EMPTY.DIR_Name[i] = dname[i];
		}

		EMPTY.DIR_Attr = ATTR_DIRECTORY;
		EMPTY.DIR_NTRes = 0;
		EMPTY.DIR_FileSize = 0;
		...
		offset = getunusedentry(newcluster);
		for(int i = 0; i < 12; i++) {
			if(i == 0) {
				EMPTY.DIR_Name[0] = '.';
			} else {
		...
		EMPTY.DIR_NTRes = 0;
		EMPTY.DIR_FileSize = 0;
		EMPTY.DIR_FstClusHI = 0;
		EMPTY.DIR_FstClusLO = 0;
		fseek(file, offset, SEEK_SET);
		fwrite(&EMPTY, sizeof(struct DIR), 1, file);
...
int rmdir(uint currentcluster, char* dir_name) {
	long offset;
	uint newcluster;
	char dname[12];
	int temp;
...
int size(uint currentcluster, char* entry_name) {
	

	char ename[12];
	int temp;

	for(int i = 0; entry_name[i] != '\0'; i++) { // same file name code from create()
		entry_name[i] = toupper(entry_name[i]);
	}

	for(int i = 0; i < 8; i++) {
		if(entry_name[i] != '\0' && entry_name[i] != '.') {
			ename[i] = entry_name[i];
		} else {
			temp = i;
			break;
		}
	...
	if(entry_name[temp++] == '.') {
		for(int i = 8; i < 11; i++) {
			if(entry_name[temp] != '\0') {
				ename[i] = entry_name[temp++];
			} else {
				temp = i;
				break;
			}
	...
	DIR = setdir(currentcluster, ename);

	//printf(" SIZE [%s] [%s]\n", entry_name, DIR.DIR_Name);

	if(DIR.DIR_Attr == ATTR_ARCHIVE) {
		printf(" %s is %d bytes.\n",entry_name,DIR.DIR_FileSize);
		return RESULT_OK;
	} else if(DIR.DIR_Attr == ATTR_DIRECTORY) {
		printf(" %s has %d records.\n",entry_name,getrecords(getdircluster(currentcluster, entry_name)));
		return RESULT_OK;
	} else {
		return RESULT_ERROR;
	}
